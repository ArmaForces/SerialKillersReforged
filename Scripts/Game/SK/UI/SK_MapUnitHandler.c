class SK_MapUnitHandler : SCR_MapMarkerDynamicWComponent
{
	...
	void CreateDots()
	{
		...
		for (int i = 0; i < dots; i++)
		{
			...
			Faction faction = m_DotMarkerEnt.GetFaction();
			if (faction && faction.GetFactionKey() != "CIV")
			{
				if (faction.GetFactionKey() == "US")
					dot.SetColor(Color.DarkBlue);
				else
					dot.SetColor(Color.Red);
			}
			else
				dot.SetColor(Color.DarkGreen);
